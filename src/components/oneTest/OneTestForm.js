...
export default function OneTestForm({ item, refetchItems, nameItem, handleCheckAnswers }) {
  ...
  return (
    <>
      <Card>
        <Link
          to={generatePath(nameItem === QUESTIONS ? paths.oneTest : paths.oneTest, { id: item._id })}
        >
          {item.url && (
            <Card.Img
              title="Open details"
